...
def parse_headers(str_headers):
    ...
    for i in str_headers.split('|'):
        try:
            dict_headers[i.split(':')[0]] = i.split(':')[1]
        except:
            pass
    return (dict_headers)


headers = parse_headers("sec-ch-ua:'Chromium';v='118', 'Google Chrome';v='118', 'Not=A?Brand';v='99'|sec-ch-ua-mobile:?0|sec-ch-ua-platform:'Windows'|DNT:1|Upgrade-Insecure-Requests:1|User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36|Accept:text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7|Sec-Fetch-Site:same-origin|Sec-Fetch-Mode:navigate|Sec-Fetch-User:?1|Sec-Fetch-Dest:document|Referer:https://www.javbus.com/search/%E7%8A%AF%E3%81%95%E3%82%8C%E3%81%9F%E5%A5%B3%E6%A0%BC%E9%97%98%E5%AE%B6&type=&parent=ce|Accept-Encoding:gzip, deflate, br|Accept-Language:zh-CN,zh;q=0.9,en;q=0.8|Cookie:PHPSESSID=94g1msmm9evi8spg6jsf4bcd03; existmag=all; genreinfo=glyphicon%20glyphicon-minus; 4fJN_2132_saltkey=Qeb2HPxE; 4fJN_2132_lastvisit=1699442947; 4fJN_2132_sid=Ja9arP; 4fJN_2132_lastact=1699446547%09home.php%09misc|")
...
class Javbus(Parser):
    ...
    def search(self, number):
        ...
        try:
            ...
            try:
                ...
                self.htmlcode = requests.get(self.detailurl, headers=headers).text
                if '<title>404 Page Not Found' in self.htmlcode \
                    or '<title>未找到页面' in self.htmlcode \
                    or '404 Not Found' in self.htmlcode \
                    or '<title>404' in self.htmlcode \
                    or '<title>お探しの商品が見つかりません' in self.htmlcode:
                    self.htmlcode = 404
                #print(self.htmlcode)
            except:
    ...
    def getOutline(self, htmltree):
        if self.morestoryline:
            from .storyline import getStoryline_avno1
            tmp = getStoryline_avno1(self.number, self.getUncensored(htmltree),
